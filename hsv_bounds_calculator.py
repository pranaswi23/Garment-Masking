...
# Create HSV range sliders (replacing trackbars)
l_h = widgets.IntSlider(min=0, max=179, value=0, description='L - H')
l_s = widgets.IntSlider(min=0, max=255, value=0, description='L - S')
l_v = widgets.IntSlider(min=0, max=255, value=0, description='L - V')
u_h = widgets.IntSlider(min=0, max=179, value=179, description='U - H')
u_s = widgets.IntSlider(min=0, max=255, value=255, description='U - S')
u_v = widgets.IntSlider(min=0, max=255, value=255, description='U - V')

# Display the sliders in a vertical layout
display(widgets.VBox([l_h, l_s, l_v, u_h, u_s, u_v]))
...
for image_file in image_files:
    ...
    while True:
        ...
        lower_range = np.array([l_h.value, l_s.value, l_v.value])
        upper_range = np.array([u_h.value, u_s.value, u_v.value])
        ...
        cv2_imshow(cv2.resize(stacked, None, fx=0.4, fy=0.4))
